...
import React, { useState, useRef, Suspense } from "react"; // 1. Added Suspense import
import { useSearchParams, useRouter } from "next/navigation";

// 2. Rename your main function to something like OtpForm
function OtpForm() {
  const [otp, setOtp] = useState<string[]>(Array(6).fill(""));
  const inputRefs = useRef<(HTMLInputElement | null)[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        "https://recipe-api-theta.vercel.app/api/v1/auth/verify-otp",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  return (
    <div className="flex items-center justify-center bg-[#E7D2C8]">
      <div className="w-full min-h-screen">
        <h2 className="text-2xl font-bold text-center text-black pt-6">
        ...
        <div className="flex justify-center">
          <button
            ...
            className="bg-[#ED6B06] text-black w-20 rounded-lg font-semibold hover:bg-orange-600 transition py-2 mx-auto"
          >
            Verify
          </button>
        </div>
...
// 3. This is the new default export that Vercel needs
